...
public class NukkitScheduler implements BukkitScheduler {
     ...
     */
    private final ConcurrentHashMap<Integer, NukkitTask> runners = new ConcurrentHashMap<Integer, NukkitTask>();
    private volatile int currentTick = -1;
    private final Executor executor = Executors.newCachedThreadPool(new com.google.common.util.concurrent.ThreadFactoryBuilder().setNameFormat("Craft NukkitScheduler Thread - %1$d").build()); // Spigot
    private NukkitAsyncDebugger debugHead = new NukkitAsyncDebugger(-1, null, null) {@Override StringBuilder debugTo(StringBuilder string) {return string;}};
